...
int main(int argc, char** argv)
{
	...
	//在目的文件夹中创建相应的文件夹，以便存入图片
		std::string str = "resultImages3/" + name[i];
		...
		// processing images
		// 初始化参数
		PatchMatchRedBlack patch_match(alpha, gamma, tau_c, tau_g);
		...
		/* int iter = 1; */
			...
			try
			{
				...
				cv::imwrite( "resultImages3/" + name[i] + "/" + name[i] + "_disp1.png", disp1);
				cv::imwrite( "resultImages3/" + name[i] + "/" + name[i] + "_disp5.png", disp2);
